...
export default class OceanScene {
    ...
    init() {
        ...
        this.stats = Experience.instance;
        if (this.stats.oceanPane && this.stats.waterPane) {
            this.stats.oceanPane.addBinding(this.parameters, 'elevation', { min: 0, max: 90, step: 0.1, label: 'Elevation' }).on('change', this.updateSun);
            this.stats.oceanPane.addBinding(this.parameters, 'azimuth', { min: -180, max: 180, step: 0.1, label: 'Azimuth' }).on('change', this.updateSun);
            this.stats.oceanPane.addBinding(this.parameters, 'turbidity', { min: 0, max: 20, step: 0.1, label: 'Turbidity' }).on('change', this.updateSun);
            this.stats.oceanPane.addBinding(this.parameters, 'rayleigh', { min: 0, max: 4, step: 0.001, label: 'Rayleigh' }).on('change', this.updateSun);
            this.stats.oceanPane.addBinding(this.parameters, 'mieCoefficient', { min: 0, max: 0.1, step: 0.001, label: 'Mie Coefficient' }).on('change', this.updateSun);
            this.stats.oceanPane.addBinding(this.parameters, 'mieDirectionalG', { min: 0, max: 1, step: 0.001, label: 'Mie Directional G' }).on('change', this.updateSun);
            this.stats.oceanPane.addBinding(this.parameters, 'sunCycle', { label: 'Day Cycle' }).on(() =>
                'change', () => { this.toggleSunCycle; }
            );
            this.stats.oceanPane.addBinding(this.parameters, 'isFreeMoving', { label: 'Move Freely' }).on(() =>
                'onChange', () => {
            ...
            this.stats.oceanPane.addBinding(this.parameters, 'boatMovement', { label: 'Look Around' }).on(() =>
                'onChange', () => {

                    this.parameters.boatMovement = !this.parameters.boatMovement;
                });
            this.stats.oceanPane.addBinding(this.parameters, 'automaticMovement', { label: 'Automatic Movement' }).on(() =>
                'onChange', () => {

                    this.parameters.automaticMovement = !this.parameters.automaticMovement;
                });

            const waterUniforms = this.water.material.uniforms;
            this.stats.waterPane.addBinding(waterUniforms.distortionScale, 'value', { min: 0, max: 8, step: 0.1, label: 'Distortion Scale' });
            this.stats.waterPane.addBinding(waterUniforms.size, 'value', { min: 0.1, max: 10, step: 0.1, label: 'Size' });
        }
        else {
